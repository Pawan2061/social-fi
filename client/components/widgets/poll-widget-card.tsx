...
import type {
  PollWidgetItem,
  WidgetPollOption,
} from "@/types/widget/widget-types";
...
export default function PollWidgetCard({
  ...
  const totalVotes = widget.pollOptions.reduce(
    (acc, o) => acc + (o._count?.PollVote ?? 0),
    0
  );

  const votedId = widget.votedOptionId ?? null;
  const disabled =
    widget.hasVoted || !!votedId || isVoting || widget.status !== "ACTIVE";
      ...
      <CardHeader className="border-b-4 border-black p-4">
        <div className="flex items-center gap-3">
          ...
          {widget.expiresAt && widget.status === "ACTIVE" && (
            <PollCountdown expiresAt={widget.expiresAt} onExpire={onRefresh} />
          )}
        </div>
      </CardHeader>
      <CardContent className="p-4 space-y-3">
        ...
        <div className="space-y-2">
          {widget.pollOptions.map((opt) => {
            ...
            const isExpired = widget.status === "EXPIRED";
            const isWinner =
              isExpired &&
              totalVotes > 0 &&
              percentage === Math.max(...widget.pollOptions.map((o) => pct(o)));

            return (
              <button
                key={opt.id}
                disabled={disabled}
                onClick={() => !disabled && onVote(widget.id, opt.id)}
                className={`w-full text-left border-2 border-black p-3 font-extrabold shadow-[3px_3px_0_0_#000] transition-all ${
                  isVoted
                    ? "bg-yellow-300"
                    : isWinner
                    ? "bg-green-300"
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm">
                    {percentage}% ({opt._count?.PollVote ?? 0})
                  </span>
                </div>
          ...
          <div className="text-xs font-bold text-gray-700">
            ...
            {widget.status === "EXPIRED" && totalVotes > 0 && (
              <span className="ml-2 text-green-600">• Poll completed</span>
            )}
          </div>
        </div>
      </CardContent>
